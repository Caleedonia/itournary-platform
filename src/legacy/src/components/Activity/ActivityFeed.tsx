...
const ActivityFeed: React.FC<ActivityFeedProps> = ({
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`/api/activity?experienceId=${experienceId}`);
        if (response.ok) {
          const data = await response.json();
          // Make sure we have valid userName in all activities
          const processedActivities = (data.activities || []).map((activity: any) => ({
            ...activity,
            userName: activity.userName || activity.user || "Unknown User"
          }));
          setActivities(processedActivities);
          setHasMore(processedActivities.length >= maxItems);
        }
      } catch (error) {
    ...
    // Only fetch if no initial activities were provided
    if (initialActivities.length === 0) {
      fetchActivities();
    }
  }, [experienceId, initialActivities.length, maxItems]);
  ...
  const getActivityMessage = (activity: ActivityItem): React.ReactNode => {
    ...
    switch (type) {
      case 'create':
        return (
          <>
            <span className={styles.userName}>{userName || "Unknown User"}</span> created 
            {details?.itemType && (
      ...
      case 'update':
        return (
          <>
            <span className={styles.userName}>{userName || "Unknown User"}</span> updated 
            {details?.itemType && (
              <span className={styles.itemType}> {details.itemType}</span>
            )}
            {details?.itemName && (
      ...
      case 'delete':
        return (
          <>
            <span className={styles.userName}>{userName || "Unknown User"}</span> deleted 
            {details?.itemType && (
              <span className={styles.itemType}> {details.itemType}</span>
            )}
            {details?.itemName && (
      ...
      case 'share':
        return (
          <>
            <span className={styles.userName}>{userName || "Unknown User"}</span> shared this experience
            {details?.message && (
      ...
      case 'comment':
        return (
          <>
            <span className={styles.userName}>{userName || "Unknown User"}</span> commented
            {details?.itemType && (
      ...
      case 'join':
        return (
          <>
            <span className={styles.userName}>{userName || "Unknown User"}</span> joined as a collaborator
          </>
        );
      default:
        return null;
